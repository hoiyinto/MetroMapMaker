...
public class AppFileController {
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
		...
		//dialog.show(props.getProperty(NEW_COMPLETED_TITLE), props.getProperty(NEW_COMPLETED_MESSAGE));
                ...
                dialog2.setTitle("New Map Request");
                dialog2.setHeaderText("Create a New Map");
                dialog2.setContentText("Please enter a name for your map:");

                // Traditional way to get the response value.
                Optional<String> result = dialog2.showAndWait();
                if (result.isPresent()){
                    //crate a directory in the export folder with its name
                    File file = new File("./export/"+(result.get()));
                    file.mkdir();
                    name=(result.get());
                }
            }
